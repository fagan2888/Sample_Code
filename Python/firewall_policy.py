...
def make_firewall_policy(config):
    ...
    firewall_policies = firewall_list(config)
    #Deal with the case if there is not firewall policy
    if (firewall_policies.__len__() < 1):
        raise TypeError("No firewall policy\n")

    rules = []
    for policy in firewall_policies.itervalues():
        # TODO - build the individual rules
        rule = firewall_rule(policy)
        rules.append(rule)
        pass
...
def firewall_rule(policy):
    ...
    if (block_all(policy) == true):
        ...
        #Check srcmac,dstmac,dstport,srcport,srcip,dstip,srcip
