* File name: c_1_Fig6.4_paperFig5C_update_PWT.do
...
quietly{
	...
	foreach file of local files{
		...
		keep `tvar' wtottradesumprod sumunempw
		...
		* Merge in PWT weighted data
		import excel "original\growth_pwt_weo2018apr_data.xlsx", sheet("pppgdp") firstrow clear
		
		n di"`file'"
		if "`file'" == "adv" {
			keep Year AdvancedeconomiesadvPerca
			rename (Year AdvancedeconomiesadvPerca) (year p3pc)
		}
		else {
			keep Year EMDEsemdPercapitagrowth
			rename (Year EMDEsemdPercapitagrowth) (year p3pc)
		}
		...
		********************************
		...
		foreach x of local vars{
			...
			preserve
				...
				if `m'==1{
					export excel using "`outputFolder'/c_1_Fig6.4_paperFig5C_update_PWT.xlsx", sheet("`gr'_`x'") firstrow(varlabel) replace
				}
				else{
					export excel using "`outputFolder'/c_1_Fig6.4_paperFig5C_update_PWT.xlsx", sheet("`gr'_`x'") firstrow(varlabel)
				}
