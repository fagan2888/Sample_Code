* File name: c_1_Fig6.3_paperFig5B_update.do
* Description: use QUARTERLY data to update AK's book Fig 6.3 (paper Fig 5.B)
	...
	*drop if year>=2015
	...
	local vars "psc spr hsepr nir rir infl"
		...
		preserve
			...
			sort tvar
			if `m'==1{
				export excel using "`outputFolder'/c_1_Fig6.3_paperFig5B_update.xlsx", sheet("`x'") firstrow(varlabel) replace
			}
			else{
				export excel using "`outputFolder'/c_1_Fig6.3_paperFig5B_update.xlsx", sheet("`x'") firstrow(varlabel)
			}
