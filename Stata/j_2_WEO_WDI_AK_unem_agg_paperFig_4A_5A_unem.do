* File name: j_2_WEO_WDI_AK_unem_agg.do
...
quietly{
	...
	* Aggregate by labor force
	...
	export excel "output/j_2_WEO_WDI_AK_unem_agg.xlsx", firstrow(variables) replace
	...
	* Event study
	...
	foreach x of local vars{
		...
		preserve
			...
			sort tvar
			if `m'==1{
				export excel using "`outputFolder'/c_1_Fig5.1_paperFig4A_WDI_WEO_AK_`x'_j_2.xlsx", sheet("`x'") firstrow(varlabel) replace
			}
			else{
				export excel using "`outputFolder'/c_1_Fig5.1_paperFig4A_WDI_WEO_AK_`x'_j_2.xlsx", sheet("`x'") firstrow(varlabel)
			}
