* File name: c_1_Fig4.1_Fig4.2_paperFig2_Fig3_update_WDI_constSamples_nonRestricted_test_datasetForR
...
quietly{
	...
	* 1 Read in data from WEO
	use "output/WDI_gdp_pop_extended.dta",replace
	...
	* Lebanon
	replace ngdpr=. if ccode=="LBN" & year<=1992
	
	* Saudi Arabia
	replace ngdpr=. if ccode=="SAU" & year<=1992
	...
	*** Prepare datasets to calculate PPP weights
	...
	foreach var in "ngdpr" "lp" "pppgdp"{
		preserve
			...
			export delimited using "output/c_1_fig3_WDI_pppweight_`var'_nonRestricted_LBN_SAU.csv", replace
		restore
	}
}	
