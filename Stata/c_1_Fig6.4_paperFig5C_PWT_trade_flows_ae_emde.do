* File name: c_1_Fig6.4_paperFig5C_PWT_trade_flows.do
* Description: use YEARLY data to update AK's book Fig 5.2 Trade flows (paper Fig 5A)
...
quietly{
	...
	foreach g of local group{
		...
		use "output/h_1_PennWorldTable_trade_flows_`g'.dta", clear
		...
		foreach x of local vars{
			...
			preserve
				...
				sort tvar
				if `m'==1{
					export excel using "`outputFolder'/c_1_Fig6.4_paperFig5C_PWT_trade_flows_`g'.xlsx", sheet("`x'") firstrow(varlabel) replace
				}
				else{
					export excel using "`outputFolder'/c_1_Fig6.4_paperFig5C_PWT_trade_flows_`g'.xlsx", sheet("`x'") firstrow(varlabel)
				}
	...
	} // End of "foreach g of local group"	
