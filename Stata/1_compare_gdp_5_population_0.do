* File name: 1_compare_gdp_5_population.do
...
quietly{
	...
	*collapse (mean) pop_yoy [aw=population], by(date) // to verify
	...
	tsline gdp* pop* if outlier != 1
	*tsline count*
	
	export excel using "`outputFolder'/`date'_1_compare_gdp_5_population.xlsx", firstrow(variables) replace
}
